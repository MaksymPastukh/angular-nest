...
export const AdminQuestionsStore = signalStore({ providedIn: 'root' }, withState(initialState), withComputed((store) => ({
    ...
    const load = rxMethod(pipe(tap(() => setPending('list')), switchMap(() => questionsService
        .getQuestions({
        page: store.page(),
    ...
    const goToPage = rxMethod(pipe(tap(({ page, pageSize }) => {
        ...
        setPending('list');
    }), switchMap(({ page, pageSize }) => questionsService
        .getQuestions({
        page,
        pageSize,
        status: store.filter().status,
    ...
    const answer = rxMethod(pipe(tap(() => setPending('submit')), exhaustMap(({ id, answer }) => questionsService.answerQuestion(id, { answer }).pipe(tapResponse({
        next: (updated) => {
            patchState(store, {
...
//# sourceMappingURL=admin-questions.store.js.map